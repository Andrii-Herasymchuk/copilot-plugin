---
name: suppa-entity
description: Manage Entities, Fields, Enums, and Schema on the Suppa platform (modern.suppa.me) via direct HTTP API calls. Use whenever the user wants to create an entity, add fields, define enum values, list or describe entities, inspect custom fields, or search records of any entity. Triggers (English): create entity, add field, add enum value, list field types, list/describe entities, show schema, custom fields, search records. Triggers (Ukrainian): створити сутність, додати поле, значення enum, типи полів, список сутностей, опиши сутність, схема сутності, кастомні поля, знайти записи, пошук записів. Works via plain HTTP — no MCP server required.
---
