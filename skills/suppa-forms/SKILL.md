---
name: suppa-forms
description: Create, update, list, and manage Dynamic Forms on the Suppa platform (modern.suppa.me) via direct HTTP API calls. Use whenever the user wants to create or build a form, define a form schema, add fields, configure form settings, or build entity forms, dashboards, or webforms. Triggers (English): create/build form, form schema, add field to form, form settings, entity form, custom form, dashboard, webform, form layout, form builder, form conditions, form validation. Triggers (Ukrainian): створити/побудувати форму, схема форми, додати поле до форми, налаштування форми, форма сутності, дашборд, вебформа, конструктор форм, умови форми, валідація форми. Works via plain HTTP — no MCP server required.
---
