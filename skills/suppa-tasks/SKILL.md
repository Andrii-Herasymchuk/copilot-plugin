---
name: suppa-tasks
description: Manage Tasks, Comments, Users, Workflows, and Stages on the Suppa platform (modern.suppa.me) via direct HTTP API calls. Use whenever the user wants to create, search, update, delete, close, or move tasks, comment on a task, find "my tasks", attach a file, or list workflows/stages/task types/users. Triggers (English): search/create/update/delete/close task, add comment, my tasks, active tasks, attach file, list workflows/stages, search user, count tasks. Triggers (Ukrainian): задача, задачі, таск, мої задачі, активні задачі, створити/оновити/видалити/закрити/перенести задачу, коментар, прикріпити файл, знайти користувача, воркфлоу, стадії, типи задач. Works via plain HTTP — no MCP server required.
---
